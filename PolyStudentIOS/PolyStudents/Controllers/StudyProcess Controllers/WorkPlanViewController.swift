...
class WorkPlanViewController: UITableViewController, SFSafariViewControllerDelegate {
    ...
    func filterArray(_ array: WorkPlan?) {
        if let array = array?.data.rup {
            if !array.isEmpty {
                if let section = array[0].sections {
                    filteredArray = section
                }
            }
        }
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if !filteredArray.isEmpty {
            ...
            if let sem = filteredArray[indexPath.row].sem {
                cell.semester.text = "Семестр: " + sem
            } else {
                cell.semester.text = ""
            }
    ...
    @objc func toggle() {
        ...
        UIView.animate(withDuration: 0.5, animations: {
            ...
            if isOpen {
                ...
                self.navigationItem.title = "Рабочий план"
                self.navigationItem.titleView?.layoutIfNeeded()
            } else {
...
extension WorkPlanViewController: UISearchBarDelegate {
