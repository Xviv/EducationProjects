...
class SecretChatLogController: UICollectionViewController, UITextFieldDelegate, UICollectionViewDelegateFlowLayout, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
        ...
        didSet {
            ...
            Database.database().reference().child("users").child(userId!).observeSingleEvent(of: .value) { (snapshot) in
                ...
                if status == "true" {
                    ...
                    self.navigationItem.setTitle(title: (self.user?.name)!, subtitle: typing)
                }
            }
    ...
    func observeMessages() {
        ...
        let userMessagesRef = Database.database().reference().child("user-secretMessages").child(uid).child(toId)
        userMessagesRef.observe(.childAdded, with: { (snapshot) in
            
            let messageId = snapshot.key
            let messagesRef = Database.database().reference().child("secretMessages").child(messageId)
    ...
    lazy var inputContainerView: ChatInputContainerView = {
        ...
        chatInputContainerView.secretChatLogController = self
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.secretChatLogController = self
    ...
    fileprivate func sendMessageWithProperties(_ properties: [String: AnyObject]) {
        let ref = Database.database().reference().child("secretMessages")
        ...
        childRef.updateChildValues(values) { (error, ref) in
            ...
            let userMessagesRef = Database.database().reference().child("user-secretMessages").child(fromId).child(toId)
            
            let messageId = childRef.key
            userMessagesRef.updateChildValues([messageId: 1])
            
            let recipientUserMessagesRef = Database.database().reference().child("user-secretMessages").child(toId).child(fromId)
    ...
    func setupNotification(deviceId:String) {
        ...
        let body = inputContainerView.inputTextField.text!
