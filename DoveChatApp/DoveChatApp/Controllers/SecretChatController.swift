...
class SecretChatController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if let chatPartnerId = message.chatPartnerId() {
            Database.database().reference().child("user-secretMessages").child(uid).child(chatPartnerId).removeValue(completionBlock: { (error, ref) in
    ...
    func observeUserMessages() {
        ...
        let ref = Database.database().reference().child("user-secretMessages").child(uid)
        ref.observe(.childAdded, with: { (snapshot) in
            
            let userId = snapshot.key
            Database.database().reference().child("user-secretMessages").child(uid).child(userId).observe(.childAdded, with: { (snapshot) in
    ...
    fileprivate func fetchMessageWithMessageId(_ messageId: String) {
        let messagesReference = Database.database().reference().child("secretMessages").child(messageId)
    ...
    @objc func handleNewMessage() {
        let newMessageController = SecretChatNewMessageController()
        newMessageController.secretChatController = self
    ...
    func showChatControllerForUser(_ user: User) {
        let chatLogController = SecretChatLogController(collectionViewLayout: UICollectionViewFlowLayout())
